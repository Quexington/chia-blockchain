...
def generate_safe_puzzle(source_pubkey: G1Element, target_pubkey: G1Element, claim_height: int, preimage_hash: bytes32) -> Program:
    return MOD.curry(bytes(source_pubkey),bytes(target_pubkey),claim_height.to_bytes(4,"big"),preimage_hash)

def get_safe_transaction_puzzle_hash(source_pubkey: G1Element, target_pubkey: G1Element, claim_height: int, preimage_hash: bytes32) -> bytes32:
    return generate_safe_puzzle(source_pubkey,target_pubkey,claim_height,preimage_hash).get_tree_hash()

# target == 0 will act as though this is the source attempting to claim it back, all other values will require a valid preimage
def generate_safe_solution(preimage: bytes32, target: int, delegated_puzzle: Program, delegated_solution: Program) -> Program:
...
def create_safe_spend_bundle_from_standard_coins(
        coins: List[Coin],synthetic_sk: PrivateKey,
        lock_amount: int,
        source_pubkey: G1Element,
        target_pubkey: G1Element,
        claim_height: int,
        preimage_hash: bytes32) -> SpendBundle:
    ...
    for coin in coins:
        ...
        first_coin = False
    spend_bundle["coin_solutions"] = coin_solutions
...
def create_claim_spend_bundle(
    ...
    claim_sk: PrivateKey,
    safe_puzzle: Program,
    safe_solution: Program,
    delegated_puzzle_hash: bytes32) -> SpendBundle:
    ...
    spend_bundle = {
        "aggregated_signature": str(sig),
        "coin_solutions": [coin_solution]
    }
...
#         })
#     ],
#     calculate_synthetic_secret_key(master_sk_to_wallet_sk(PrivateKey.from_bytes(bytes.fromhex("0xmastersk")),0),DEFAULT_HIDDEN_PUZZLE_HASH),
